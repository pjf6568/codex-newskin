...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-newskin.ps1` once. The installer preserves the user's native appearance settings, seeds the Arina Hashimoto theme, copies the runtime to `%LOCALAPPDATA%\CodexNewskin\engine`, and creates launch/restore/tray shortcuts that do not depend on the source checkout.
2. Use the `Codex Newskin` shortcut, or run `%LOCALAPPDATA%\CodexNewskin\engine\scripts\start-newskin.ps1`. The shortcut asks before restarting an already-open Codex app; CLI callers must explicitly add `-RestartExisting`.
3. Run `scripts/verify-newskin.ps1 -ScreenshotPath <absolute-path>` after launch. Treat a missing continuous wallpaper, home shell, native composer, sidebar layer, or injection marker as failure. The native suggestion count is responsive and may be two to four.
...
- Keep the injection daemon running for navigation/reload resilience. Its state and logs live under `%LOCALAPPDATA%\CodexNewskin`.
...
- The active theme, saved themes, imported images, pause marker, and tray controls live under `%LOCALAPPDATA%\CodexNewskin`. Reject empty or over-16 MB images before copying or encoding them.
...
- Treat `%LOCALAPPDATA%\CodexNewskin\engine` as an installer-managed runtime. Exit the Newskin tray before reinstalling so the installer can replace that runtime atomically and update every shortcut to the same copy.
