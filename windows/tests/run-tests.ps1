...
try {
  ...
  $zoneMarkedSourceScript = Join-Path $runtimeSourceRoot 'scripts\start-newskin.ps1'
  ...
  $engine = Install-NewskinRuntimeEngine -SkillRoot $runtimeSourceRoot -StateRoot $runtimeStateRoot
  ...
  if ($runtimeSourceFiles.Count -ne $runtimeEngineFiles.Count -or
    -not (Test-NewskinPathWithin -Path $engine.Start -Root $runtimeStateRoot) -or
    -not (Test-NewskinPathWithin -Path $engine.Restore -Root $runtimeStateRoot) -or
    -not (Test-NewskinPathWithin -Path $engine.Tray -Root $runtimeStateRoot)) {
  ...
  $realRuntimeCleanup = (Get-Command Remove-NewskinRuntimeTree -CommandType Function).ScriptBlock
  ...
  try {
    ...
    if (-not $runtimeCleanupFailure.Triggered -or $runtimeUpdateReportedFailure -or
      ...
      (Read-NewskinUtf8File -Path (Join-Path $engine.Root 'scripts\runtime-update.test')) -cne 'updated') {
      throw 'Runtime reinstall did not commit cleanly when old-engine cleanup failed.'
    }
  } finally {
    $WarningPreference = $previousWarningPreference
    Set-Item -Path Function:\Remove-NewskinRuntimeTree -Value $realRuntimeCleanup
  }
  foreach ($runtimeBackup in Get-ChildItem -LiteralPath $runtimeStateRoot -Directory -Force |
    ...
    Remove-NewskinRuntimeTree -Path $runtimeBackup.FullName -StateRoot $runtimeStateRoot
  }
  ...
  Remove-Item -LiteralPath (Join-Path $invalidRuntimeRoot 'scripts\start-newskin.ps1') -Force
  $invalidRuntimeRejected = $false
  try {
    $null = Install-NewskinRuntimeEngine -SkillRoot $invalidRuntimeRoot -StateRoot $runtimeStateRoot
  } catch {
  ...
  try {
    $null = Install-NewskinRuntimeEngine -SkillRoot $runtimeSourceRoot -StateRoot $nestedStateRoot
  } catch {
  ...
  $installSource = Read-NewskinUtf8File -Path (Join-Path $Root 'scripts\install-newskin.ps1')
  $commonSource = Read-NewskinUtf8File -Path (Join-Path $Root 'scripts\common-windows.ps1')
  $hashVerificationIndex = $commonSource.IndexOf(
    'Staged Newskin runtime failed hash verification', [System.StringComparison]::Ordinal
  )
  ...
  $trayGuardIndex = $installSource.IndexOf('if (Test-NewskinTrayActive)', [System.StringComparison]::Ordinal)
  $engineInstallIndex = $installSource.IndexOf('$engine = Install-NewskinRuntimeEngine', [System.StringComparison]::Ordinal)
  ...
  Write-NewskinUtf8FileAtomically -Path $atomicReplacePath -Content 'after'
  if ((Read-NewskinUtf8File -Path $atomicReplacePath) -cne 'after') {
  ...
  $realAtomicCleanup = (Get-Command Remove-NewskinAtomicArtifact -CommandType Function).ScriptBlock
  $previousWarningPreference = $WarningPreference
  ...
  try {
    ...
    try {
      Write-NewskinUtf8FileAtomically -Path $cleanupFailurePath -Content 'after'
    } catch {
      $cleanupFailureReported = $true
    }
    if (-not $cleanupFailure.Triggered -or $cleanupFailureReported -or
      (Read-NewskinUtf8File -Path $cleanupFailurePath) -cne 'after') {
    ...
    Install-NewskinBaseTheme -ConfigPath $cleanupConfigPath -BackupPath $cleanupBackupPath
    if (-not (Test-Path -LiteralPath $cleanupBackupPath) -or
      -not (Test-NewskinBytesEqual -Left $cleanupOriginalBytes `
    ...
    $WarningPreference = $previousWarningPreference
    Set-Item -Path Function:\Remove-NewskinAtomicArtifact -Value $realAtomicCleanup
  }

  $configPath = Join-Path $temporaryRoot 'config.toml'
  $backupPath = Join-Path $temporaryRoot 'config.before-newskin.toml'
  ...
  $installed = Read-NewskinUtf8File -Path $configPath
  ...
  if (-not (Test-Path -LiteralPath (Get-NewskinAppearanceMarkerPath -BackupPath $backupPath))) {
  ...
  Write-NewskinUtf8FileAtomically -Path $configPath -Content $installed
  Restore-NewskinBaseTheme -ConfigPath $configPath -BackupPath $backupPath
  $restored = Read-NewskinUtf8File -Path $configPath
  ...
  $legacyCurrent = "[desktop]`r`n$($script:NewskinLegacyAppearanceTheme)`r`n$($script:NewskinManagedLightCodeTheme)`r`n$($script:NewskinManagedLightChromeTheme)`r`n"
  ...
  Install-NewskinBaseTheme -ConfigPath $legacyConfigPath -BackupPath $legacyBackupPath
  $legacyMigrated = Read-NewskinUtf8File -Path $legacyConfigPath
  ...
  Write-NewskinUtf8FileAtomically -Path $legacyConfigPath -Content $legacyMigrated
  Restore-NewskinBaseTheme -ConfigPath $legacyConfigPath -BackupPath $legacyBackupPath
  if ((Read-NewskinUtf8File -Path $legacyConfigPath) -notmatch 'appearanceTheme = "dark"') {
  ...
  Install-NewskinBaseTheme -ConfigPath $lfConfigPath -BackupPath $lfBackupPath
  $lfInstalled = Read-NewskinUtf8File -Path $lfConfigPath
  ...
  Restore-NewskinBaseTheme -ConfigPath $lfConfigPath -BackupPath $lfBackupPath
  $lfRestored = Read-NewskinUtf8File -Path $lfConfigPath
  ...
  Install-NewskinBaseTheme -ConfigPath $quotedConfigPath -BackupPath $quotedBackupPath
  $quotedInstalled = Read-NewskinUtf8File -Path $quotedConfigPath
  ...
  Restore-NewskinBaseTheme -ConfigPath $quotedConfigPath -BackupPath $quotedBackupPath
  if ((Read-NewskinUtf8File -Path $quotedConfigPath) -cne $quotedOriginal) {
  ...
  Install-NewskinBaseTheme -ConfigPath $nestedConfigPath -BackupPath $nestedBackupPath
  $nestedInstalled = Read-NewskinUtf8File -Path $nestedConfigPath
  $nestedDesktop = Get-NewskinDesktopSection -Content $nestedInstalled
  ...
  Restore-NewskinBaseTheme -ConfigPath $nestedConfigPath -BackupPath $nestedBackupPath
  if ((Read-NewskinUtf8File -Path $nestedConfigPath) -cne $nestedOriginal) {
  ...
  Install-NewskinBaseTheme -ConfigPath $singleLineArrayPath -BackupPath $singleLineArrayBackup
  if (-not (Read-NewskinUtf8File -Path $singleLineArrayPath).Contains($singleLineArray.TrimEnd())) {
  ...
  foreach ($unsupported in @(
    ...
    try { Install-NewskinBaseTheme -ConfigPath $unsupportedPath -BackupPath $unsupportedBackup } catch { $unsupportedRejected = $true }
  ...
  Write-NewskinUtf8FileAtomically -Path $configPath -Content 'intentionally changed'
  Restore-NewskinConfigBackup -ConfigPath $configPath -BackupPath $backupPath -RecoveryBackupPath $recoveryPath
  ...
  if ((Read-NewskinUtf8File -Path $recoveryPath) -cne 'intentionally changed') {
  ...
  Archive-NewskinConfigBackup -BackupPath $backupPath -ArchivePath $archivePath
  ...
  Install-NewskinBaseTheme -ConfigPath $configPath -BackupPath $backupPath
  if (-not (Test-NewskinBytesEqual -Left $secondBaselineBytes -Right ([System.IO.File]::ReadAllBytes($backupPath)))) {
  ...
  $rejected = $false
  try { Install-NewskinBaseTheme -ConfigPath $invalidPath -BackupPath $invalidBackupPath } catch { $rejected = $true }
  ...
  $utf16Rejected = $false
  try { Install-NewskinBaseTheme -ConfigPath $utf16Path -BackupPath $utf16BackupPath } catch { $utf16Rejected = $true }
  ...
  try { Install-NewskinBaseTheme -ConfigPath $utf16NoBomPath -BackupPath $utf16NoBomBackupPath } catch { $utf16NoBomRejected = $true }
  ...
  $raceRejected = $false
  try { Assert-NewskinFileUnchanged -Path $racePath -ExpectedBytes $raceExpected } catch { $raceRejected = $true }
  if (-not $raceRejected) { throw 'Concurrent config modification was not detected.' }
  $conditionalWriteRejected = $false
  try {
    Write-NewskinUtf8FileAtomically -Path $racePath -Content 'replacement' -ExpectedBytes $raceExpected
  } catch {
    $conditionalWriteRejected = $true
  }
  if (-not $conditionalWriteRejected -or (Read-NewskinUtf8File -Path $racePath) -cne 'after') {
    throw 'Conditional atomic write replaced newer config content.'
  }

  if (-not (Test-NewskinWebSocketUrl -Value 'ws://127.0.0.1:9335/devtools/page/test' -Port 9335)) {
  ...
  foreach ($unsafe in @(
    ...
    if (Test-NewskinWebSocketUrl -Value $unsafe -Port 9335) { throw "Accepted unsafe CDP target: $unsafe" }
  }
  ...
  if (-not (Test-NewskinCdpPageTarget -Target $safePageTarget -Port 9335)) {
  ...
  foreach ($unsafePageTarget in @(
    ...
    if (Test-NewskinCdpPageTarget -Target $unsafePageTarget -Port 9335) {
      throw 'Accepted an inconsistent CDP page target.'
    }
  }
  $watchCommand = '"C:\Program Files\nodejs\node.exe" "C:\Newskin\injector.mjs" --watch --port 9335 --browser-id browser-123'
  if (-not (Test-NewskinCommandLineToken -CommandLine $watchCommand -Token 'C:\Newskin\injector.mjs') -or
    (Test-NewskinCommandLineToken -CommandLine $watchCommand -Token 'Newskin\injector.mjs')) {
    throw 'Injector command-line token validation is not boundary-safe.'
  }
  if (-not (Test-NewskinBrowserId -Value 'browser-123') -or
    (Test-NewskinBrowserId -Value 'browser 123')) {
    throw 'CDP browser ID validation is not boundary-safe.'
  }
  $quotedProfile = ConvertTo-NewskinProcessArgument -Value '--user-data-dir=C:\Newskin\Profile\'
  if ($quotedProfile -cne '"--user-data-dir=C:\Newskin\Profile\\"') {
    throw 'Process argument quoting did not protect spaces and a trailing backslash.'
  }
  $argumentLine = ConvertTo-NewskinArgumentLine -Arguments @(
    '--remote-debugging-address=127.0.0.1',
    '--user-data-dir=C:\Newskin\Profile\',
    ''
  )
  if ($argumentLine -cne '--remote-debugging-address=127.0.0.1 "--user-data-dir=C:\Newskin\Profile\\" ""') {
    throw 'Packaged-app argument line quoting failed.'
  }
  Initialize-NewskinPackageLauncher
  if (-not ('CodexNewskin.PackageLauncher' -as [type])) {
    throw 'Packaged-app activation helper did not compile.'
  }
  $invalidActivationRejected = $false
  try { $null = Start-NewskinCodex -Codex ([pscustomobject]@{ AppUserModelId = 'invalid app' }) } catch {
  ...
  $state = [pscustomobject]@{
    ...
    injectorPath = 'C:\Newskin\injector.mjs'
  ...
  $loadedState = Read-NewskinState -Path $statePath
  ...
  Write-NewskinState -Path $statePath -State $missingIdentityState
  $missingIdentityRejected = $false
  try { $null = Read-NewskinState -Path $statePath } catch { $missingIdentityRejected = $true }
  ...
  Write-NewskinState -Path $statePath -State $legacyState
  if ((Read-NewskinState -Path $statePath).schemaVersion -ne 2) {
  ...
  $fakeInstall = ConvertTo-NewskinCodexInstall -Package $fakePackage -Manifest $fakeManifest
  if ($null -eq $fakeInstall -or $fakeInstall.PackageFullName -cne $fakePackage.PackageFullName -or
    ...
    -not (Test-NewskinPathEqual -Left $fakeInstall.Executable -Right $fakeExecutable)) {
    throw 'Registered Appx package identity conversion failed.'
  }
  $fakeManifest.Package.Applications.Application[1].Id = 'Invalid App'
  if ($null -ne (ConvertTo-NewskinCodexInstall -Package $fakePackage -Manifest $fakeManifest)) {
  ...
  if ($null -eq (Get-NewskinCodexStatePathCandidate -State $pathOnlyState)) {
    throw 'A structurally valid legacy Codex path was not recognized for read-only activity checks.'
  }
  if ($null -eq (Resolve-NewskinCodexInstallFromState -State $pathOnlyState `
  ...
  $resolvedInstall = Resolve-NewskinCodexInstallFromState -State $verifiedPackageState `
    -RegisteredInstalls @($fakeInstall)
  ...
  if ($null -ne (Resolve-NewskinCodexInstallFromState -State $verifiedPackageState `
    -RegisteredInstalls @($fakeInstall))) {
    throw 'A mismatched Appx package family was accepted from state.'
  }
  Write-NewskinUtf8FileAtomically -Path $statePath -Content '[]'
  $badStateRejected = $false
  try { $null = Read-NewskinState -Path $statePath } catch { $badStateRejected = $true }
  ...
  $themePaths = Initialize-NewskinThemeStore -SkillRoot $Root -StateRoot $themeStateRoot
  ...
  $initialTheme = Read-NewskinTheme -ThemeDirectory $themePaths.Active
  ...
  if ($updatedTheme.Theme.name -cne '测试主题' -or
    ...
    -not (Test-NewskinThemePathWithin -Path $updatedTheme.ImagePath -Root $themePaths.Active)) {
  ...
  try {
    $null = Use-NewskinSavedTheme -ThemeDirectory $junctionTheme -StateRoot $themeStateRoot
  ...
  Set-NewskinPaused -Paused $true -StateRoot $themeStateRoot | Out-Null
  if (-not (Test-NewskinPaused -StateRoot $themeStateRoot)) { throw 'Pause marker was not created.' }
  Set-NewskinPaused -Paused $false -StateRoot $themeStateRoot | Out-Null
  if (Test-NewskinPaused -StateRoot $themeStateRoot) { throw 'Pause marker was not removed.' }
  ...
  Write-NewskinUtf8FileAtomically -Path (Join-Path $oversizedTheme 'theme.json') `
    -Content "{`"image`":`"oversized.jpg`"}`r`n"
  $oversizedReadRejected = $false
  try { $null = Read-NewskinTheme -ThemeDirectory $oversizedTheme } catch { $oversizedReadRejected = $true }
  $oversizedSetRejected = $false
  try {
    $null = Set-NewskinActiveTheme -ImagePath $oversizedImage -Theme $null -StateRoot $themeStateRoot
  ...
  try { $null = Set-NewskinActiveTheme -ImagePath $oversizedDimensionImage -Theme $null -StateRoot $themeStateRoot } catch { $oversizedDimensionRejected = $true }
  ...
  try { $null = Initialize-NewskinThemeStore -SkillRoot $Root -StateRoot $reparseStateRoot } catch { $reparseInitRejected = $true }
  ...
  $css = Read-NewskinUtf8File -Path (Join-Path $Root 'assets\newskin.css')
  ...
  $traySource = Read-NewskinUtf8File -Path (Join-Path $Root 'scripts\tray-newskin.ps1')
  ...
  if (-not $traySource.Contains('Read-NewskinTheme -ThemeDirectory $paths.Active -SkipImageMetadata') -or
    -not $traySource.Contains('Get-NewskinSavedThemes -StateRoot $StateRoot -SkipImageMetadata')) {
    throw 'Tray menu metadata enumeration still performs full image parsing on every open.'
  }
  $restoreSource = Read-NewskinUtf8File -Path (Join-Path $Root 'scripts\restore-newskin.ps1')
  if (-not $restoreSource.Contains('Stop-NewskinTrayProcess')) {
  ...
  if ($restoreSource.Contains('Start-Process -FilePath $relaunchCodex.Executable') -or
    -not $restoreSource.Contains('Start-NewskinCodex -Codex $relaunchCodex')) {
    throw 'Restore still executes the WindowsApps path instead of activating the registered package.'
  }
  $startSource = Read-NewskinUtf8File -Path (Join-Path $Root 'scripts\start-newskin.ps1')
  if ($startSource.Contains('Start-Process -FilePath $codex.Executable') -or
    -not $startSource.Contains('Start-NewskinCodex -Codex $codex')) {
    throw 'Start still executes the WindowsApps path instead of activating the registered package.'
  }
  $stateReadIndex = $startSource.IndexOf('$previousState = Read-NewskinState', [System.StringComparison]::Ordinal)
  $restartPromptIndex = $startSource.IndexOf('$restartAuthorized = Confirm-NewskinRestart', [System.StringComparison]::Ordinal)
  $recordedStopIndex = $startSource.IndexOf('$recordedInjectorStopped = Stop-NewskinRecordedInjector', [System.StringComparison]::Ordinal)
  $cancelIndex = $startSource.IndexOf("Write-Host 'Newskin launch was cancelled", [System.StringComparison]::Ordinal)
  $pauseClearIndex = $startSource.IndexOf('Set-NewskinPaused -Paused $false', [System.StringComparison]::Ordinal)
  ...
  if (-not $startSource.Contains('$pauseWasSet = Test-NewskinPaused') -or
    -not $startSource.Contains('$pauseCleared = $true') -or
    -not $startSource.Contains('Set-NewskinPaused -Paused $true -StateRoot $StateRoot')) {
    throw 'Start does not preserve an existing pause marker when startup rolls back.'
  }

  $rendererSource = Read-NewskinUtf8File -Path (Join-Path $Root 'assets\renderer-inject.js')
  ...
  $injectorSource = Read-NewskinUtf8File -Path (Join-Path $Root 'scripts\injector.mjs')
  ...
  $themeSource = Read-NewskinUtf8File -Path (Join-Path $Root 'scripts\theme-windows.ps1')
  foreach ($requiredThemeSafety in @(
    '[System.IO.FileAttributes]::ReparsePoint',
    'Ensure-NewskinManagedDirectory',
    'Get-NewskinValidatedImageMetadata',
  ...
  $stderrProbe = Invoke-NewskinNative -FilePath $node.Path -ArgumentList @(
    '-e', "process.stderr.write('newskin-stderr-probe\n'); process.exit(7)")
  if ($stderrProbe.ExitCode -ne 7 -or ($stderrProbe.Output -join "`n") -notmatch 'newskin-stderr-probe') {
  ...
  $discardedProbe = Invoke-NewskinNative -FilePath $node.Path -ArgumentList @(
  ...
  $oversizedPayloadTest = Invoke-NewskinNative -FilePath $node.Path -ArgumentList @(
  ...
  $rendererTest = Invoke-NewskinNative -FilePath $node.Path -ArgumentList @(
  ...
  $bootstrapTest = Invoke-NewskinNative -FilePath $node.Path -ArgumentList @(
  ...
  $oneShotTest = Invoke-NewskinNative -FilePath $node.Path -ArgumentList @(
  ...
  $imageMetadataTest = Invoke-NewskinNative -FilePath $node.Path -ArgumentList @(
