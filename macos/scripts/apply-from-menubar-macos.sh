...
on run argv
  ...
  display dialog promptText buttons {"取消", okLabel} default button okLabel with title "ChatGPT Newskin"
end run
APPLESCRIPT
}
...
THEME_NAME=""
PORT="9341"
if [ -x "$SCRIPT_DIR/status-newskin-macos.sh" ]; then
  ...
  done < <("$SCRIPT_DIR/status-newskin-macos.sh" 2>/dev/null)
fi
