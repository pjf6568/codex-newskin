...
// Newskin CSS auto-adapts to light/dark via data-dream-shell.
...
async function acquireConfigLock() {
  const lockPath = `${configPath}.newskin.lock`;
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error("Another Newskin config operation is still running; try again shortly.");
      }
