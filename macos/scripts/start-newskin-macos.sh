...
release_start_lock() {
  ...
  /bin/rm -f "$START_LOCK_DIR/pid" 2>/dev/null || true
  /bin/rmdir "$START_LOCK_DIR" 2>/dev/null || true
  START_LOCK_HELD="false"
}

acquire_start_lock() {
  START_LOCK_DIR="$STATE_ROOT/.start-once.lock"
  if /bin/mkdir "$START_LOCK_DIR" 2>/dev/null; then
    printf '%s\n' "$$" > "$START_LOCK_DIR/pid"
    START_LOCK_HELD="true"
    return 0
  fi
  local owner=""
  owner="$(/bin/cat "$START_LOCK_DIR/pid" 2>/dev/null || true)"
  case "$owner" in
    ''|*[!0-9]*) ;;
    *)
      if /bin/kill -0 "$owner" 2>/dev/null; then
        fail "A Codex Newskin start is already in progress (PID $owner); refusing a second launch or restart."
      fi
      ;;
  esac
  # A dead owner can leave an empty lock after a crash. Clear only this exact,
  # per-user temporary lock, then make one atomic retry; never wait or retry a
  # launch in a loop.
  /bin/rm -f "$START_LOCK_DIR/pid" 2>/dev/null || true
  /bin/rmdir "$START_LOCK_DIR" 2>/dev/null || true
  if /bin/mkdir "$START_LOCK_DIR" 2>/dev/null; then
  ...
  fail "Could not acquire the one-shot Codex Newskin start lock; no launch was attempted."
}

has_verified_active_session() {
  [ -f "$STATE_PATH" ] || return 1
  [ "$(state_field session 2>/dev/null || true)" = "active" ] || return 1
  verified_cdp_endpoint "$PORT" 2>/dev/null || return 1
  local recorded_pid recorded_start recorded_node recorded_path recorded_port
...
record_start_exit() {
  ...
  printf 'ChatGPT Newskin: start failed at line %s (exit %s). See %s\n' "$line" "$code" "$START_ERROR_LOG" >&2
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --refresh-running) REFRESH_RUNNING="true"; shift ;;
...
# A second launcher click after a successful start is a no-op. In particular,
# do not recycle the injector, close Codex, or attempt another app launch.
if [ "$REFRESH_RUNNING" != "true" ] && has_verified_active_session; then
  "$SCRIPT_DIR/sync-terminal-theme-macos.sh" --theme-dir "$THEME_DIR" --quiet >/dev/null 2>&1 || true
  printf 'Codex Newskin is already active on loopback port %s; no second launch or restart was attempted.\n' "$PORT"
  exit 0
fi

if [ "$REFRESH_RUNNING" = "true" ] && [ "$DEBUG_READY" = "true" ]; then
  printf 'Refreshing the active Newskin injector on loopback port %s; Codex will not be closed or relaunched.\n' "$PORT"
fi
...
if codex_is_running && [ "$DEBUG_READY" = "false" ]; then
  if [ "$PROMPT_RESTART" = "true" ] && [ "$RESTART_EXISTING" = "false" ]; then
    if ! /usr/bin/osascript -e 'display dialog "ChatGPT 需要重启一次才能启用皮肤。通常会在 10–30 秒内完成。" buttons {"取消", "重启并应用"} default button "重启并应用" with title "ChatGPT Newskin"' >/dev/null; then
...
if [ "$DEBUG_READY" = "false" ]; then
  ...
  # launch_codex_with_cdp already starts exactly one fresh app instance. A
  # second `open -na` here creates a competing renderer with the same debug
  # port; newer Codex builds can then repeatedly close/reopen while the two
  # instances contend for the single profile and CDP listener.
...
VERIFY_OUTPUT="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/newskin-verify.XXXXXX")"
...
OPERATION_FINISHED="true"
printf 'ChatGPT Newskin %s is active on loopback port %s.\n' "$SKIN_VERSION" "$PORT"
