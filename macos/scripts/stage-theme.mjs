...
async function main() {
  ...
  if (![1, 2].includes(theme?.schemaVersion) || typeof theme.image !== "string" || !theme.image) {
    throw new Error("Theme config has an unsupported schema or media field");
  }
  const mediaType = theme.mediaType ?? "image";
  if (mediaType !== "image" && mediaType !== "video") {
    throw new Error("Theme mediaType must be image or video");
  }
  const mediaExtension = path.extname(theme.image).toLowerCase();
  if (!(mediaType === "video" ? VIDEO_EXTENSIONS : IMAGE_EXTENSIONS).has(mediaExtension)) {
    throw new Error(`Theme ${mediaType} has an unsupported format: ${mediaExtension || "missing"}`);
  }
  ...
  const media = await readStableFile(
    imagePath,
    `Theme ${mediaType}`,
    mediaType === "video" ? MAX_VIDEO_BYTES : MAX_IMAGE_BYTES,
  );
  if (media.bytes.length < 1) throw new Error(`Theme ${mediaType} is empty`);

  let banner = null;
  const bannerName = theme?.home?.banner;
  if (bannerName !== undefined) {
    if (typeof bannerName !== "string" || !bannerName || path.basename(bannerName) !== bannerName ||
      /[\u0000-\u001f\u007f-\u009f\u2028\u2029]/u.test(bannerName)) {
      throw new Error("Theme banner must be a filename inside its theme directory");
    }
    if (bannerName !== theme.image) {
      const bannerPath = path.resolve(sourceRoot, bannerName);
      assertContained(sourceRoot, bannerPath, "Theme banner");
      banner = { name: bannerName, ...(await readStableFile(bannerPath, "Theme banner", MAX_IMAGE_BYTES)) };
      if (banner.bytes.length < 1) throw new Error("Theme banner is empty");
    }
  }
  ...
  await writeExclusive(path.join(stageRoot, theme.image), media.bytes);
  if (banner) await writeExclusive(path.join(stageRoot, banner.name), banner.bytes);
