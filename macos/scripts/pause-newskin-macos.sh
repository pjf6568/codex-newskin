...
if [ "$REMOVED" = "true" ]; then
  printf 'ChatGPT Newskin paused (skin removed; control connection %s). Port %s may still be in debug mode.\n' \
    "$([ "$KEEP_CONTROL_WATCHER" = "true" ] && printf retained || printf unavailable)" "$PORT"
elif codex_is_running; then
  printf 'ChatGPT Newskin paused (injector stopped). Live remove skipped: CDP on port %s not verified.\n' "$PORT"
else
  printf 'ChatGPT Newskin paused (ChatGPT is not running).\n'
fi
