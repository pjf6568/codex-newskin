...
if [ "$IN_PLACE" = "false" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  exec "$INSTALL_ROOT/scripts/install-newskin-macos.sh" "${install_args[@]}"
fi
...
write_launcher() {
  local target="$1"
  local command="$2"
  if [ -e "$target" ] && ! /usr/bin/grep -q '^# CodexNewskinStudio launcher$' "$target" 2>/dev/null; then
    fail "Refusing to overwrite an unrelated Desktop file: $target"
  fi
  /usr/bin/printf '%s\n' \
    '#!/bin/bash' \
    '# CodexNewskinStudio launcher' \
    'set -e' \
    "$command" > "$target"
...
if [ "$LAUNCH_AFTER_INSTALL" = "true" ]; then
  "$SCRIPT_DIR/start-newskin-macos.sh" --port "$PORT" --prompt-restart
fi
