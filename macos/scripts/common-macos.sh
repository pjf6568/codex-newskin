...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf 'ChatGPT Newskin: could not resolve the current macOS home directory.\n' >&2; exit 1; }
  export HOME
fi
...
INSTALL_ROOT="$HOME/.codex/codex-newskin"
STATE_ROOT="$HOME/Library/Application Support/CodexNewskinStudio"
...
CODEX_APP_JOB_LABEL="com.openai.codex-newskin.app"
INJECTOR_JOB_LABEL="com.openai.codex-newskin.injector"
...
SKIN_VERSION="1.2.0"
NEWSKIN_VALIDATED_RUNTIME_PID=""
NEWSKIN_VALIDATED_RUNTIME_BUNDLE=""
NEWSKIN_VALIDATED_RUNTIME_EXE=""
NEWSKIN_VALIDATED_RUNTIME_NODE=""
...
on run argv
  display notification (item 1 of argv) with title "ChatGPT Newskin"
end run
APPLESCRIPT
}
...
alert_user() {
  ...
  /usr/bin/osascript - "$message" <<'APPLESCRIPT' >/dev/null 2>&1 || true
on run argv
  display alert "ChatGPT Newskin" message (item 1 of argv)
end run
APPLESCRIPT
}
...
remember_validated_runtime_identity() {
  NEWSKIN_VALIDATED_RUNTIME_PID="$$"
  NEWSKIN_VALIDATED_RUNTIME_BUNDLE="$CODEX_BUNDLE"
  NEWSKIN_VALIDATED_RUNTIME_EXE="$CODEX_EXE"
  NEWSKIN_VALIDATED_RUNTIME_NODE="$NODE"
}
...
legacy_zh_process_started_at() {
  LC_ALL=zh_CN.UTF-8 /bin/ps -p "$1" -o lstart= 2>/dev/null | /usr/bin/awk '{$1=$1; print}'
}
...
recorded_injector_process_matches() {
  ...
  if [ -z "$actual_start" ] || [ "$actual_start" != "$expected_start" ]; then
    # Newskin 1.2.0 wrote lstart under the caller's locale. Accept only
    # the known Chinese legacy spelling after every other process identity
    # check above has passed; all newly written states use the C locale.
    [ "$expected_start" = "$(legacy_zh_process_started_at "$pid")" ] || return 1
  fi
  return 0
}
...
stop_recorded_injector() {
  ...
  if ! pid="$(state_field injectorPid 2>/dev/null)" || [ -z "${pid:-}" ]; then
    printf 'Newskin state is damaged or missing its injector PID; state was preserved.\n' >&2
    return 1
  fi
  ...
  case "$pid" in
    *[!0-9]*|??????????*)
      printf 'Recorded Newskin injector PID is invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  ...
  case "$saved_port" in
    ''|*[!0-9]*)
      printf 'Recorded Newskin injector port is missing or invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  [ "$saved_port" -ge 1024 ] && [ "$saved_port" -le 65535 ] || {
    printf 'Recorded Newskin injector port is out of range; state was preserved.\n' >&2
    return 1
  }
  if [ -z "$saved_start" ] || [ -z "$saved_node" ] || [ -z "$saved_injector" ]; then
    printf 'Recorded Newskin injector identity is incomplete; state was preserved.\n' >&2
    return 1
  fi
  ...
  if recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    printf 'Could not stop the recorded Newskin injector (PID %s).\n' "$pid" >&2
    return 1
  fi
  return 0
}
...
ensure_node_runtime() {
  if [ "$NEWSKIN_VALIDATED_RUNTIME_PID" = "$$" ] \
    && [ -n "$NEWSKIN_VALIDATED_RUNTIME_NODE" ] \
    && [ "${NODE:-}" = "$NEWSKIN_VALIDATED_RUNTIME_NODE" ] \
    && [ "${CODEX_BUNDLE:-}" = "$NEWSKIN_VALIDATED_RUNTIME_BUNDLE" ] \
    && [ "${CODEX_EXE:-}" = "$NEWSKIN_VALIDATED_RUNTIME_EXE" ]; then
    return 0
  fi
  discover_codex_app
  require_signed_node_runtime
