...
ENGINE="$CLIENT_ROOT/.codex-newskin"
...
# Keep the customer ZIP self-contained and translate repository-only Windows
# links for the hidden standalone engine.
...
/usr/bin/printf '%s\n' \
  ...
  'set -euo pipefail' \
  'ROOT="$(cd "$(dirname "$0")" && pwd -P)"' \
  'exec "$ROOT/.codex-newskin/scripts/install-newskin-macos.sh"' \
