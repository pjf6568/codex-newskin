...
PLUGIN_SRC="$PROJECT_ROOT/menubar/codex_newskin.10s.sh"
...
PLUGIN_DST="$PLUGIN_DIR/codex_newskin.10s.sh"
{
  printf '%s\n' '#!/bin/bash'
  printf 'export CODEX_NEWSKIN_ENGINE=%q\n' "$ENGINE_ROOT"
...
if [ -d "$INSTALL_ROOT" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  /bin/cp -f "$PLUGIN_SRC" "$INSTALL_ROOT/menubar/codex_newskin.10s.sh"
  /bin/chmod 755 "$INSTALL_ROOT/menubar/codex_newskin.10s.sh"
  for name in pause-newskin-macos.sh status-newskin-macos.sh apply-from-menubar-macos.sh \
...
/bin/chmod 755 \
  "$PROJECT_ROOT/scripts/pause-newskin-macos.sh" \
  "$PROJECT_ROOT/scripts/status-newskin-macos.sh" \
