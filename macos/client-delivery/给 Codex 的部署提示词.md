...
2. 解压后的客户目录根部应能看到：
   ...
   - `使用说明.txt`
   - `给 Codex 的部署提示词.md`
   完整引擎位于隐藏目录 `.codex-newskin`。这是正常结构，不要删除、改名或只复制其中的 CSS/图片。Finder 默认看不到隐藏目录时，直接从终端使用其绝对路径。
...
7. 启动后必须运行：
   - `~/.codex/codex-newskin/scripts/doctor-macos.sh --require-live`
...
9. 如果失败，读取 `~/Library/Application Support/CodexNewskinStudio/` 下的日志并继续修复。不得降低代码签名、回环端口归属、PID 身份、原生结构或截图验证标准；不得用“预计重启后生效”“应该完成”等措辞代替验收。
