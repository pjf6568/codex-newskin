...
Turn an image or local video you like into one continuous full-window Codex theme. The same media runs beneath the native sidebar and main surface, while route-aware translucency keeps home, task, plugin, scheduled-task, and pull-request controls fully interactive and readable.
...
| Item | Path |
| --- | --- |
| Engine | `~/.codex/codex-newskin` |
| State / logs / user images | `~/Library/Application Support/CodexNewskinStudio` |
...
That ZIP contains a visible installer plus a hidden `.codex-newskin` engine. Do not ship only CSS/images.

## How it works (security boundary)

1. Discover `com.openai.codex` and validate signature / Team ID / arch / bundled Node.
2. Start one normal Codex process with CDP bound to `127.0.0.1` only; no launchd job owns or relaunches Codex.
...
MP4, WebM, and MOV files up to 32 MB are supported as local, muted, looping backgrounds. MP4 (H.264) or WebM is recommended for reliable Chromium playback. Import one without changing the official app:

```bash
~/.codex/codex-newskin/scripts/customize-theme-macos.sh \
  --video "/absolute/path/background.mp4" \
  --name "动态主题"
```

Video themes use `mediaType: "video"` in `theme.json`; their `image` field remains the local media filename for compatibility with the existing theme store. A video cannot use `home.banner`.
...
The Arina Hashimoto pack contains a standardized `2560 × 1440` JPEG plus theme
metadata. The artwork is a user-provided AI-generated example, not an official
OpenAI/Codex visual or endorsement; confirm likeness and asset rights before
redistributing it.
...
- Start with a `2560 × 1440` pure background, then use the theme metadata to
  tune focus, safe area, and task-page treatment.
...
```bash
~/.codex/codex-newskin/scripts/load-image-theme-macos.sh \
...
```bash
~/.codex/codex-newskin/scripts/customize-theme-macos.sh --reset-demo
```
