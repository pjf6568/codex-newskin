...
const tempRoot = await fs.mkdtemp(path.join("/tmp", "codex-newskin-stage-"));
...
try {
  ...
  const videoSource = path.join(tempRoot, "themes", "video");
  const videoStage = path.join(tempRoot, "video-stage");
  await fs.mkdir(videoSource, { recursive: true });
  await fs.mkdir(videoStage);
  await fs.writeFile(path.join(videoSource, "background.mp4"), Buffer.from("video-fixture"));
  await fs.writeFile(
    path.join(videoSource, "theme.json"),
    `${JSON.stringify({ schemaVersion: 2, id: "video", image: "background.mp4", mediaType: "video" })}\n`,
  );
  assert.equal(await runStage(videoSource, videoStage), "background.mp4");
  assert.deepEqual(await fs.readFile(path.join(videoStage, "background.mp4")), Buffer.from("video-fixture"));
