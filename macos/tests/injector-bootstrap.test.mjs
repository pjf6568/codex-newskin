...
assert.equal(generations.context.window.__CODEX_NEWSKIN_EARLY_APPLIED__, "new");
...
assert.match(
  source,
  /request\.action === "select-native"[\s\S]{0,720}pause-newskin-macos\.sh/,
  "Selecting native mode from the page must use the persisted pause transaction, not an in-memory-only cleanup.",
);
assert.match(
  source,
  /const homePass = !result\.homeRoute \|\| result\.homePresent;/,
  "Volatile empty-home presentation markup must not cause a false theme-application failure.",
);
assert.match(
  source,
  /url\.searchParams\.get\("initialRoute"\) === AVATAR_OVERLAY_ROUTE[\s\S]{0,180}searchParams\.keys\(\)\]\.length === 1/,
  "Only the exact avatar-overlay route may receive the transparent pet-host treatment.",
);
assert.match(
  source,
  /html, body, #root, #app[\s\S]{0,180}background: transparent !important;[\s\S]{0,180}\.codex-avatar-root, \.codex-avatar-button \{ background-color: transparent !important; \}/,
  "The overlay canvas must be transparent while the sprite host keeps its background-image layer.",
);
assert.match(
  source,
  /spriteStyle\?\.backgroundImage[\s\S]{0,180}setProperty\("background-image", spriteStyle\.backgroundImage\)/,
  "The transparent-host migration must restore the native avatar sprite image if it was previously reset.",
);
