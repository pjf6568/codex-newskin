...
- 菜单栏与 `status-newskin-macos.sh` 不再依赖 `/usr/bin/python3`（macOS 12.3+ 默认不预装）读取主题名与运行状态，改用纯 shell 解析；此前在未装 Xcode 命令行工具的机器上，主题名会退化成 id、`--json` 状态直接失效
...
- 暂停与完全恢复时清理 `com.openai.codex-newskin.app` 作业
